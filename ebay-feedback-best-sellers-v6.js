...
function period(r){
  ...
  if(/\bpast 6 months\b|\bwithin the past 6 months\b|\b6 months\b|\b6 mesi\b|\bsei mesi\b|\bpast year\b|\bwithin the past year\b|\blast year\b|\b1 year\b|\bone year\b|\bpast 12 months\b|\b12 months\b|\b12 mesi\b|\bultimo anno\b|\b1 anno\b|\bmore than a year\b|\bover a year\b|\bmore than 1 year\b|\bover 1 year\b|\bpiu di un anno\b|\boltre un anno\b|\banni? fa\b|\bmonths ago\b|\bmesi fa\b/.test(t)){
    return"older";
  }
...
function periodTotals(){
  let month=0;
  for(const x of products.values())month+=x.monthCount||0;
  return{month};
}
...
<div class="h">
  <div>
    ...
    <div class="sub">Analisi Last Month · ${V}</div>
  </div>
...
<div class="b">
  ...
  <div class="official">
    <span class="chip">Positive Last Month: <b>${Number.isFinite(official.month)?official.month:"—"}</b></span>
  </div>
    ...
    <table>
      <thead>
        <tr>
          ...
          <th class="num" data-sort="month" title="Ordina per 1 month">Last Month <span class="sort-arrow"></span></th>
...
function render(){
  ...
  $("#tb").innerHTML=a.length
    ...
    `).join("")
    : '<tr><td colspan="5">Nessun prodotto rilevato.</td></tr>';
}

function process(d){
  let monthRows=0,olderRows=0,unknownRows=0;
  let posMonth=0;
  ...
  for(const r of fbRows(d)){
    ...
    if(per!=="month"||!positive(r))continue;
    posMonth++;
    ...
    if(!q){
      ...
      if(title)recoveryRows.push({feedbackId:fid,title,period:"month"});
      continue;
    }
  ...
  render();
  return{monthRows,olderRows,unknownRows,posMonth,total:fbRows(d).length};
}
...
try{
  ...
  for(let i=1;i<=150&&!stop;i++){
    ...
    log(`Pagina ${i}: +${r.posMonth} Positive Last Month · ${products.size} prodotti.`);

    $("#status").textContent=`Fase 1 · pagina ${i} · prodotti ${products.size} · Last Month ${periodTotals().month}`;

    if(trackedWindowSeen&&r.olderRows>0){
      log(`Fine Last Month rilevata alla pagina ${i}. Stop feedback.`);
      break;
    }
  ...
  $("#status").textContent=stop
    ...
    : `Completato · ${products.size} prodotti · recuperati da titolo ${recoveredFromTitle} · ancora senza ID ${unmapped} · Last Month ${t.month} · Item sold ${soldFound}/${soldDone} · totale ${totalItemSold()}`;
