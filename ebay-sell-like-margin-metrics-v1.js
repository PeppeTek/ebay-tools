...
const PATCH_ID='capitan-margin-metrics-v3';
...
function updateSalePriceLabel(){const r=rows().find(x=>/^Prezzo(?: di vendita \(-2%\))?:/i.test(clean(x.innerText||x.textContent)));if(!r)return;const b=r.querySelector('b');if(b)b.textContent='Prezzo di vendita (-2%):';const span=r.querySelector('span');if(span){const t=clean(span.textContent||'');if(/^[0-9]+(?:[.,][0-9]+)?\s*\(-2%\)$/i.test(t))span.textContent=t.replace(/\s*\(-2%\)$/i,' USD (-2%)')}}
...
function ensureRows(){
  ...
  const defs=[
    ['capitan-margin-break','Margine fino al Break Even'],
    ['capitan-margin-amazon','Margine sul costo Amazon'],
    ['capitan-margin-delta','Scostamento dal Break Even']
  ];
  let anchor=be;
...
function setMetric(p,id,val){const el=p.querySelector('#'+id+' [data-value]');if(!el)return;el.textContent=money(val);el.style.fontWeight=isFinite(val)?'700':'400';el.style.color=isFinite(val)?(val<0?'#b42318':'#137333'):''}
function refresh(){
  const p=ensureRows();if(!p||!hasAmazonResults())return false;
  const sale=salePrice(),be=breakEven(),cost=selectedAmazonCost();
  const marginBreak=(isFinite(sale)&&isFinite(be))?sale-be:null;
  const marginAmazon=(isFinite(sale)&&isFinite(cost))?sale-cost:null;
  const delta=(isFinite(be)&&isFinite(cost))?be-cost:null;
  setMetric(p,'capitan-margin-break',marginBreak);
  setMetric(p,'capitan-margin-amazon',marginAmazon);
  setMetric(p,'capitan-margin-delta',delta);
