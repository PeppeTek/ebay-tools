...
async function saveAndVerify(root,parts,postal){
  ...
  for(let i=0;i<30;i++){await sleep(180);if(summaryMatches(parts,postal)){try{document.querySelectorAll('[data-capitan-hidden-location]').forEach(x=>{x.style.opacity='';x.style.pointerEvents='';x.style.transition='';delete x.dataset.capitanHiddenLocation});document.querySelectorAll('[data-capitan-hidden-location-parent]').forEach(x=>{x.style.background='';delete x.dataset.capitanHiddenLocationParent})}catch(_){}return true;}}
