...
function patchUi(){
  ...
  if(!p.querySelector('[data-ebay-actions]')){
    ...
    p.appendChild(box);
    box.addEventListener('click',e=>{const b=e.target.closest('button[data-ebay-action]');if(!b)return;if(b.dataset.ebayAction==='csv')return;triggerNativeAction(b.dataset.ebayAction);});
  }
