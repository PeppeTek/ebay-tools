...
function patchUi(){
  ...
  if(!p.querySelector('[data-ebay-actions]')){
    ...
    box.addEventListener('click',async e=>{const b=e.target.closest('button[data-ebay-action]');if(!b)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();const kind=b.dataset.ebayAction;
      if(kind==='csv'){if(typeof window.__capitanDownloadVariantCsv==='function')window.__capitanDownloadVariantCsv();else alert('CSV varianti non ancora pronto.');return}
      if(kind==='preview'||kind==='list'){
        ...
        try{
          ...
          window.__capitanLog?.('Descrizione non completata: '+String(err&&err.message||err),'warn')
        }finally{
