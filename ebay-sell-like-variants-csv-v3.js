...
function taxonomyCategoryLookup(categoryId){
  ...
  const endpoint=categoryTaxonomyEndpoint();
  if(!endpoint)return Promise.reject(Error('Endpoint Apps Script Taxonomy non configurato'));

  return new Promise((resolve,reject)=>{
    const cb='__capitanTaxonomyCategoryCb_'+Date.now()+'_'+Math.floor(Math.random()*1e6);
    const s=document.createElement('script');
    const timer=setTimeout(()=>done(Error('Timeout eBay Taxonomy')),10000);
...
function currentCategoryName(){
  ...
  // 1) Exact Taxonomy result for the current Category ID.
  const taxonomy=clean(window.__capitanTaxonomyCategoryName||'');
  const taxonomyId=clean(window.__capitanTaxonomyCategoryId||'');
  if(taxonomy&&(!taxonomyId||taxonomyId===id))return taxonomy;

  // 2) CapitanShop category archive snapshot.
  if(id&&CAPITAN_CATEGORY_NAME_BY_ID[id]){
    return clean(CAPITAN_CATEGORY_NAME_BY_ID[id])
  }

  // 3) Category name already imported by the clone/variant payload.
  const d=cloneData()||{},st=variantState()||{},vd=st.data||{};
  const imported=clean(d.categoryName||vd.categoryName||window.__capitanCurrentCategoryName||'');
  if(imported){
  ...
  // 4) Exact visible ITEM CATEGORY section on the eBay editor page.
  try{
    const lines=String(document.body&&document.body.innerText||'')
      .split(/\r?\n/).map(clean).filter(Boolean);

    for(let i=0;i<lines.length;i++){
      if(!/^item category$/i.test(lines[i]))continue;
      for(let j=i+1;j<Math.min(lines.length,i+6);j++){
        const v=lines[j];
        if(!v||/^in\s+/i.test(v))continue;
        if(/^(edit|learn more|feedback)$/i.test(v))continue;
        if(v.length<=120)return v
      }
    }
  }catch(_){}

  // 5) Category link text tied to the same Category ID.
  try{
    for(const a of document.querySelectorAll('a[href]')){
      const href=String(a.href||'');
      if(id&&!href.includes(id))continue;
      const v=clean(a.innerText||a.textContent||'');
      if(v&&v.length<=120&&!/^edit$/i.test(v))return v
    }
  }catch(_){}
