...
function businessDaysUntil(date){
  ...
  if(end<=now)return 0;
  let d=new Date(now),n=0,guard=0;
...
async function sourceShippingMaxBusinessDays(sourceItemId){
  try{
    const r=await fetch('https://www.ebay.com/itm/'+encodeURIComponent(sourceItemId),{credentials:'include',cache:'no-store'});
    if(!r.ok)return null;
    const html=await r.text();
    const normalized=html.replace(/[–—]/g,'-');
    const values=[];
    let m;
    const range=/(\d{1,2})\s*(?:-|to)\s*(\d{1,2})\s*(?:business|working)\s*days?/ig;
    while((m=range.exec(normalized)))values.push(Number(m[2]));
    const single=/(\d{1,2})\s*(?:business|working)\s*days?/ig;
    while((m=single.exec(normalized)))values.push(Number(m[1]));
    const dateRe=/"(?:maxEstimatedDeliveryDate|maxDeliveryDate|estimatedDeliveryDate)"\s*:\s*"([^"]+)"/ig;
    while((m=dateRe.exec(normalized))){
      const n=businessDaysUntil(new Date(m[1]));
      if(n)values.push(n)
    }
...
function shippingPolicyForDays(days){
  days=Number(days);
  if(!Number.isFinite(days)||days<=0)return'';
  return days<=5?'Standard Shipping 1 - 5 business days +2':'Economy Shipping 1 - 10 business days +3'
}
function normalizePolicyText(v){return clean(v).replace(/\s*[\[(]\s*\d+\s+listings?\s*[\])]/ig,'').replace(/\s+/g,' ').trim()}
function shippingPolicyControl(){
  const exact=[...document.querySelectorAll('label')].find(l=>/^shipping policy$/i.test(clean(l.innerText||l.textContent||'')));
  if(exact){
    let el=exact.htmlFor?document.getElementById(exact.htmlFor):null;
    if(!el)el=exact.parentElement&&exact.parentElement.querySelector('input,textarea,select,[role="combobox"],button,[role="button"]');
    if(el)return el
  }
  const nodes=[...document.querySelectorAll('div,span,p')].filter(e=>visible(e)&&/^shipping policy$/i.test(clean(e.innerText||e.textContent||'')));
  for(const n of nodes){
    let p=n.parentElement;
    for(let i=0;i<4&&p;i++,p=p.parentElement){
      const el=p.querySelector('input,textarea,select,[role="combobox"],button,[role="button"]');
      if(el)return el
    }
  }
  return null
}
async function setShippingPolicy(target){
  target=normalizePolicyText(target);
  if(!target)return false;
  const control=shippingPolicyControl();
  if(!control)return false;
  const current=normalizePolicyText(controlValue(control)||control.innerText||control.textContent||'');
  if(current===target)return true;
  try{control.scrollIntoView({block:'center',inline:'nearest'})}catch(_){}
  try{control.click()}catch(_){}
  await sleep(250);

  const candidates=[...document.querySelectorAll('[role="option"],[role="menuitemradio"],li,button,div,span')]
    .filter(visible)
    .map(e=>({e,t:normalizePolicyText(e.innerText||e.textContent||'')}))
    .filter(x=>x.t===target)
    .sort((a,b)=>(a.e.children.length-b.e.children.length));
  if(!candidates.length)return false;
  const option=candidates[0].e.closest('[role="option"],[role="menuitemradio"],li,button')||candidates[0].e;
  try{option.click()}catch(_){return false}
  await sleep(450);
  const after=normalizePolicyText(controlValue(control)||control.innerText||control.textContent||'');
  if(after===target)return true;
  const text=normalizePolicyText((control.parentElement&&control.parentElement.innerText)||'');
...
try{const ep=endpoint();if(!ep)throw Error('URL backend mancante.');status.textContent='Recupero listing sorgente e preparazione descrizione AI…';const data=await jsonp(ep);if(!data||!data.ok)throw Error(data?.error||'Risposta backend non valida');window.__capitanSellLikeCloneData=data;try{localStorage.setItem('capitan-sell-like-clone-data-v1',JSON.stringify(data))}catch(_){}let sellMode='mono';try{const mi=window.__capitanSellLikeModePromise?await Promise.race([window.__capitanSellLikeModePromise,sleep(12000).then(()=>({mode:'mono'}))]):{mode:window.__capitanSellLikeMode||'mono'};sellMode=mi&&mi.mode==='variants'?'variants':'mono'}catch(_){}window.__capitanSellLikeMode=sellMode;
const pageCategory=currentPageCategoryName();
if(pageCategory){data.categoryName=pageCategory;window.__capitanCurrentCategoryName=pageCategory}
let shippingDays=Number(data.shippingMaxBusinessDays||0)||null;
if(!shippingDays)shippingDays=await sourceShippingMaxBusinessDays(itemId);
const shippingTarget=shippingPolicyForDays(shippingDays)||clean(data.shippingPolicyTarget||'');
if(shippingTarget){data.shippingMaxBusinessDays=shippingDays;data.shippingPolicyTarget=shippingTarget}
...
add('Titolo / Item Specifics','ok','lasciati invariati');if(sellMode==='mono'){window.__capitanSellLikeSalePrice=Number(data.targetPrice);const priceOk=setPrice(data.targetPrice);add('Prezzo',priceOk?'ok':'warn',priceOk?`${Number(data.targetPrice).toFixed(2)} (-2%)`:'campo non trovato');const qtyOk=setQuantity(data.quantity);add('Quantità',qtyOk?'ok':'warn',qtyOk?String(data.quantity):'campo non trovato')}else{add('Quantità','ok',String(data.quantity||3))}
if(data.shippingPolicyTarget){const shipOk=await setShippingPolicy(data.shippingPolicyTarget);add('Shipping Policy',shipOk?'ok':'warn',shipOk?data.shippingPolicyTarget:data.shippingPolicyTarget+' — selezione automatica non confermata')}
if(sellMode==='mono'){const conditionOk=await setConditionNew();add('Condizione',conditionOk?'ok':'warn',conditionOk?'New':'controlla manualmente')}
if(data.itemLocation){const locOk=setItemLocation(data.itemLocation);add('Item Location',locOk?'ok':'warn',locOk?data.itemLocation:'sorgente: '+data.itemLocation+' — campo eBay non trovato, controlla manualmente')}else add('Item Location','warn','non recuperata dalla listing sorgente');await enableHtmlMode();const descOk=setDescription(data.descriptionHtml);add('Descrizione',descOk?'ok':'warn',descOk?'Template AI-HTML incluso':'editor HTML non trovato');status.textContent='Caricamento foto nello stesso ordine…';try{const n=await uploadImages(data.images||[]);add('Foto','ok',`${n}/${(data.images||[]).length} inviate all’uploader eBay`)}catch(e){add('Foto','warn',e.message+' — lascia la pagina aperta e verifica manualmente')}add('Pubblicazione','ok','List it lasciato manuale');status.innerHTML=sellMode==='variants'?'Generazione CSV varianti in corso…':'<span class="ok">Preparazione completata.</span> Verifica Item Location, foto, prezzo e descrizione prima di cliccare List it.';if(sellMode==='mono')stopProcessTimer()}catch(e){console.error(e);status.innerHTML='<span class="bad">Errore:</span> '+esc(e.message||e);stopProcessTimer()}
})();