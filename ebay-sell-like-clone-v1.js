...
function currentPageCategoryName(){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    for(let j=i+1;j<Math.min(lines.length,i+5);j++){
      ...
      if(!v||/^(edit|help)$/i.test(v)||/learn more|opens in a new window|sales tax/i.test(v))continue;
...
async function sourceShippingDays(sourceItemId){
  ...
  try{
    ...
    while((m=range.exec(text)))vals.push(Number(m[2]));
    const single=/(\d{1,2})\s*(?:business|working)\s*days?/ig;
    while((m=single.exec(text)))vals.push(Number(m[1]));
    const delivery=/estimated\s+between\s+(?:[A-Za-z]{3},?\s+)?([A-Za-z]{3,9}\s+\d{1,2})\s+and\s+(?:[A-Za-z]{3},?\s+)?([A-Za-z]{3,9}\s+\d{1,2})/ig;
    while((m=delivery.exec(text))){
      const y=(new Date()).getFullYear();
      let end=new Date(m[2]+' '+y);
      const today=new Date();today.setHours(0,0,0,0);
      if(!isNaN(end)&&end<today)end=new Date(m[2]+' '+(y+1));
      const n=businessDaysUntil(end);
      if(n)vals.push(n)
    }
    const valid=vals.filter(n=>Number.isFinite(n)&&n>0&&n<=60);
...
function shippingPolicyControl(){
  for(const l of document.querySelectorAll('label')){
    if(!/^shipping policy$/i.test(clean(l.innerText||l.textContent||'')))continue;
    let el=l.htmlFor?document.getElementById(l.htmlFor):null;
    if(!el&&l.parentElement)el=l.parentElement.querySelector('input,textarea,select,[role="combobox"],button,[role="button"]');
    if(el)return el
  }
  return null
}
async function applyShippingPolicy(target){
  target=normalizePolicyText(target);
  if(!target)return false;
  const control=shippingPolicyControl();
  if(!control)return false;
  const now=normalizePolicyText(controlValue(control)||control.innerText||control.textContent||'');
  if(now===target)return true;
  try{control.click()}catch(_){return false}
  await sleep(250);
  const hit=[...document.querySelectorAll('[role="option"],[role="menuitemradio"],li,button,div,span')]
    .filter(visible)
    .map(e=>({e,t:normalizePolicyText(e.innerText||e.textContent||'')}))
    .filter(x=>x.t===target)
    .sort((a,b)=>a.e.children.length-b.e.children.length)[0];
  if(!hit)return false;
  const option=hit.e.closest('[role="option"],[role="menuitemradio"],li,button')||hit.e;
  try{option.click()}catch(_){return false}
  await sleep(350);
  return normalizePolicyText(controlValue(control)||control.innerText||control.textContent||'')===target
}
function addOrUpdateShippingRow(target,state){
  const existing=[...steps.querySelectorAll('.row')].find(r=>/^Shipping Policy:/i.test(clean(r.innerText||r.textContent||'')));
  const msg=target;
  if(existing){
    existing.innerHTML='<b>Shipping Policy:</b> <span class="'+state+'">'+esc(msg)+'</span>';
    return
  }
  add('Shipping Policy',state,msg)
}
...
if(pageCategory){
  data.categoryName=pageCategory;
  window.__capitanCurrentCategoryName=pageCategory;
...
if(sellMode==='mono'){
  window.__capitanSellLikeSalePrice=Number(data.targetPrice);
  const priceOk=setPrice(data.targetPrice);
  add('Prezzo',priceOk?'ok':'warn',priceOk?`${Number(data.targetPrice).toFixed(2)} (-2%)`:'campo non trovato');
  const qtyOk=setQuantity(data.quantity);
  add('Quantità',qtyOk?'ok':'warn',qtyOk?String(data.quantity):'campo non trovato')
}else{
  add('Quantità','ok',String(data.quantity||3))
}
if(sellMode==='mono'){
  const conditionOk=await setConditionNew();
  add('Condizione',conditionOk?'ok':'warn',conditionOk?'New':'controlla manualmente')
}
if(data.itemLocation){
  const locOk=setItemLocation(data.itemLocation);
  add('Item Location',locOk?'ok':'warn',locOk?data.itemLocation:'sorgente: '+data.itemLocation+' — campo eBay non trovato, controlla manualmente')
}else add('Item Location','warn','non recuperata dalla listing sorgente');
await enableHtmlMode();
const descOk=setDescription(data.descriptionHtml);
add('Descrizione',descOk?'ok':'warn',descOk?'Template AI-HTML incluso':'editor HTML non trovato');
status.textContent='Caricamento foto nello stesso ordine…';
try{
  const n=await uploadImages(data.images||[]);
  add('Foto','ok',`${n}/${(data.images||[]).length} inviate all’uploader eBay`)
}catch(e){
  add('Foto','warn',e.message+' — lascia la pagina aperta e verifica manualmente')
}
add('Pubblicazione','ok','List it lasciato manuale');
status.innerHTML=sellMode==='variants'?'Generazione CSV varianti in corso…':'<span class="ok">Preparazione completata.</span> Verifica Item Location, foto, prezzo e descrizione prima di cliccare List it.';
if(sellMode==='mono')stopProcessTimer();

// IMPORTANT: shipping lookup is deliberately background-only.
// It can never hold up the working Sell Like flow.
Promise.resolve().then(async()=>{
  const days=await sourceShippingDays(itemId);
  const target=shippingPolicyForDays(days);
  if(!target)return;
  data.shippingMaxBusinessDays=days;
  data.shippingPolicyTarget=target;
  window.__capitanSellLikeCloneData=data;
  try{localStorage.setItem('capitan-sell-like-clone-data-v1',JSON.stringify(data))}catch(_){}
