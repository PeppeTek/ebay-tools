...
'use strict';
const V='v1.6.1';
const AI_RECOVERY_ENDPOINT=String(window.__capitanSellLikeBackendEndpoint||'').replace(/\/+$/,'');
...
async function ensureAiDescription(data){
  data=data||window.__capitanSellLikeCloneData;
  if(!data)throw Error('Dati clone non disponibili');
  if(data.descriptionHtml&&!data.aiDeferred)return data.descriptionHtml;
  if(window.__capitanAiDescriptionPromise)return await window.__capitanAiDescriptionPromise;

  window.__capitanAiDescriptionPromise=(async()=>{
    try{
      if(typeof window.__capitanLog==='function')window.__capitanLog('Generazione Template AI-HTML…','info');
      const r=await jsonpAction(AI_RECOVERY_ENDPOINT,'sell_like_ai_retry',{itemId},90000);
      if(!r||!r.ok)throw Error(r&&r.error||'Generazione descrizione AI non riuscita');
      ...
      data.descriptionHtml=r.descriptionHtml;
      data.aiModel=r.aiModel||data.aiModel||'';
      data.aiFallback=!!r.aiFallback;
      data.aiError=r.aiError||'';
      data.aiDeferred=false;
      window.__capitanSellLikeCloneData=data;
      try{localStorage.setItem('capitan-sell-like-clone-data-v1',JSON.stringify(data))}catch(_){}
      try{window.dispatchEvent(new CustomEvent('capitan-ai-description-updated',{detail:{itemId,descriptionHtml:r.descriptionHtml}}))}catch(_){}
      if(typeof window.__capitanLog==='function')window.__capitanLog('Template AI-HTML incluso','ok');
      return r.descriptionHtml
    }catch(err){
      if(typeof window.__capitanLog==='function')window.__capitanLog('Descrizione AI: '+String(err&&err.message||err),'warn');
      throw err
    }finally{
      window.__capitanAiDescriptionPromise=null
    }
  })();

  return await window.__capitanAiDescriptionPromise
}
window.__capitanEnsureAiDescription=()=>ensureAiDescription(window.__capitanSellLikeCloneData);
...
try{const ep=endpoint();if(!ep)throw Error('URL backend mancante.');status.textContent='Recupero listing sorgente…';let data=await jsonp(ep);if((!data||!data.ok)&&isAiDescriptionError(data&&data.error)){status.textContent='Errore AI rilevato: continuo con la descrizione originale…';try{const recovered=await jsonpAction(AI_RECOVERY_ENDPOINT,'clone_prepare',{itemId},90000);if(recovered&&recovered.ok)data=recovered}catch(recoveryError){console.warn('Sell Like AI fallback endpoint',recoveryError)}}if(!data||!data.ok)throw Error(data?.error||'Risposta backend non valida');window.__capitanSellLikeCloneData=data;try{localStorage.setItem('capitan-sell-like-clone-data-v1',JSON.stringify(data))}catch(_){}let sellMode='mono';try{const mi=window.__capitanSellLikeModePromise?await Promise.race([window.__capitanSellLikeModePromise,sleep(12000).then(()=>({mode:'mono'}))]):{mode:window.__capitanSellLikeMode||'mono'};sellMode=mi&&mi.mode==='variants'?'variants':'mono'}catch(_){}window.__capitanSellLikeMode=sellMode;if(sellMode==='mono'){
  ...
  if(!qtyOk)add('Quantità','warn','campo non trovato')
}
const conditionOk=await setConditionNew();
if(!conditionOk)add('Condizione','warn','controlla manualmente');

if(data.itemLocation){
  const locOk=setItemLocation(data.itemLocation);
  if(!locOk)add('Item Location','warn','sorgente: '+data.itemLocation+' — campo eBay non trovato, controlla manualmente')
}else{
  add('Item Location','warn','non recuperata dalla listing sorgente')
}
...
status.innerHTML=sellMode==='variants'
  ?'Generazione CSV varianti in corso…'
  :'<span class="ok">Preparazione completata.</span>';
if(typeof window.__capitanLog==='function')window.__capitanLog('Preparazione completata','ok');
if(sellMode==='mono')stopProcessTimer()}catch(e){console.error(e);status.innerHTML='<span class="bad">Errore:</span> '+esc(e.message||e);stopProcessTimer()}
})();