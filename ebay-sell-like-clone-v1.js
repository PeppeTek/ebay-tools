...
function formatShippingAmount(v,currency){
  ...
  const sym=/^USD$/i.test(String(currency||'USD'))?'$ ':clean(currency||'USD')+' ';
  return sym+n.toFixed(2)
}
function parseSourceShippingLabel(html){
  html=String(html||'');if(!html)return'';
  const raw=html.replace(/\\u0024/gi,'
...
try{
  ...
  try{
    ...
    text=clean(doc.body&&doc.body.innerText||'')
  }catch(_){text=clean(raw.replace(/<[^>]+>/g,' '))}
  text=text.replace(/[–—]/g,'-');
