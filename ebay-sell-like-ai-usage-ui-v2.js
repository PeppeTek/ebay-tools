...
async function render(body){
  ...
  try{
    ...
    body.innerHTML=
      ...
      '</div>'+
      '<div style="font-size:11px;color:#666;margin-top:12px">Cambio USD/EUR usato: '+esc(Number(d.usdEurRate||0).toFixed(4))+'. Se il modello non ha un prezzo configurato il costo viene indicato come N/D.</div>'+
      '<div style="font-size:13px;font-weight:700;margin:18px 0 8px">Ultime chiamate AI</div>'+
      '<div id="capitan-ai-usage-table-scroll" style="overflow:auto;border:1px solid #e5e7eb;border-radius:9px;flex:1;min-height:180px;max-height:360px;margin-bottom:20px"><table style="width:100%;border-collapse:collapse;font-size:11px"><thead style="position:sticky;top:0;background:#fafafa;z-index:1"><tr style="background:#fafafa"><th style="padding:7px 5px;text-align:left">Data</th><th style="padding:7px 5px;text-align:left">Funzione</th><th style="padding:7px 5px;text-align:left">Modello</th><th style="padding:7px 5px;text-align:right">Token</th><th style="padding:7px 5px;text-align:right">Costo</th></tr></thead><tbody>'+(recent||'<tr><td colspan="5" style="padding:14px;text-align:center;color:#777">Nessuna chiamata registrata.</td></tr>')+'</tbody></table></div>'+
      '<div style="font-size:10px;color:#777;margin-top:12px;line-height:1.4">Il pannello registra solo le chiamate effettuate dopo la sua attivazione. Il saldo o la fattura Groq non sono disponibili tramite questo endpoint pubblico.</div>';
...
function patch(){
  ...
  const aiBody=document.createElement('div');aiBody.id='capitan-ai-usage-body';aiBody.style.cssText='display:none;padding:18px 22px;min-height:0;overflow:hidden;flex:1;flex-direction:column';rateBody.insertAdjacentElement('afterend',aiBody);
